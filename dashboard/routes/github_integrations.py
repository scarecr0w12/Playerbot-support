...
class GitHubIntegrationsModule:
    ...
    async def integrations_github_workflow_save(
        ...
        if default_repo and not GITHUB_REPO_RE.match(default_repo):
            raise HTTPException(status_code=400, detail="Invalid default repo format")
        if review_digest_repo and not GITHUB_REPO_RE.match(review_digest_repo):
            raise HTTPException(status_code=400, detail="Invalid review digest repo format")
        if review_digest_channel and not review_digest_channel.isdigit():
            raise HTTPException(status_code=400, detail="Review digest channel must be a numeric channel ID")
        if not review_digest_hour_utc.isdigit() or not 0 <= int(review_digest_hour_utc) <= 23:
            raise HTTPException(status_code=400, detail="Review digest hour must be between 0 and 23")
        if not review_digest_stale_hours.isdigit() or int(review_digest_stale_hours) < 1:
            raise HTTPException(status_code=400, detail="Review digest stale hours must be at least 1")
        if issue_default_template and issue_default_template not in ISSUE_TEMPLATE_KEYS:
            raise HTTPException(status_code=400, detail="Invalid issue template choice")
        milestone_values = {
        ...
        for key, value in milestone_values.items():
            if value and not value.isdigit():
                raise HTTPException(status_code=400, detail=f"{key} must be a numeric milestone number")
        ...
        if any(milestone_values.values()):
            ...
            if not milestone_repo:
                raise HTTPException(
                    status_code=400,
                    detail="Set a default repo before saving issue template milestone defaults.",
                )
            if not GITHUB_REPO_RE.match(milestone_repo):
                raise HTTPException(status_code=400, detail="Invalid default repo format for milestone validation")
